...
2. GROSS MARGIN & EBITDA: What is {brand_name}'s reported gross margin and EBITDA margin?
3. OPERATIONS: Manufacturing model, manufacturing countries, logistics/fulfillment model, key supply chain risks.
4. BUSINESS MODEL DETAILS: Revenue channel split, geographic revenue breakdown, AOV estimates, repeat purchase rate.
...
1. COMPANY BASICS: Legal name, founding year, founding CITY, current headquarters city, founders, employee count, business model, product categories.
2. FINANCIALS (CRITICAL): Search for "{brand_name} revenue FY25", "{brand_name} annual results 2025". I need: Most recent annual revenue in NATIVE CURRENCY, revenue history for 3+ years, gross margin, EBITDA, profit before tax.
3. OPERATIONS: Manufacturing model, manufacturing countries, logistics/fulfillment model.
...
2. SOCIAL MEDIA: Instagram followers, TikTok followers, Facebook, YouTube, Twitter/X current followers.
3. CUSTOMER REVIEWS: Trustpilot: Go to trustpilot.com/review/{domain} — what is the EXACT star rating and EXACT total review count? Common praise and complaint themes.
4. TECH STACK: E-commerce platform, analytics tools, email marketing provider.
...
1. DIRECT COMPETITORS: 6-8 direct competitors with estimated annual revenue, price range, key differentiator.
2. MARKET SIZE: TAM, SAM, SOM with CAGR growth rate from Grand View Research, Mordor Intelligence, Statista, or Allied Market Research.
3. M&A COMPARABLES: Recent acquisitions or PE investments in comparable companies (2020-2026). Include deal values and EV/Revenue multiples.
4. INDUSTRY TRENDS: Key trends affecting {brand_name}'s market.
...
{{
  "company": {{"legal_name": "string or null", "brand_name": "{brand_name}", "domain": "{domain}", "founded_year": "year", "founded_city": "city", "current_headquarters": "city", "founders": [{{"name": "...", "title": "...", "background": "..."}}], "employee_count": {{"value": 0, "date": "YYYY-MM", "source_id": "PB-1", "source_url": "url"}}, "business_model": "DTC / B2B / etc", "product_categories": ["cat1"], "price_range": "$X-$Y", "key_markets": ["market1"], "unique_selling_points": ["usp1"], "brand_positioning": "description", "source_id": "PB-1", "source_url": "url"}},
  "financials": {{"revenue_history": [{{"year": 2024, "revenue": "amount with currency", "growth_yoy": "X%", "source_id": "id", "source_url": "url"}}], "latest_revenue": {{"year": 2024, "amount": "amount with currency", "source_id": "id", "source_url": "url"}}, "gross_margin": {{"value": "X%", "basis": "reported/estimated", "source_id": "id", "source_url": "url"}}, "ebitda": {{"amount": "£XM", "margin": "X%", "source_id": "id", "source_url": "url"}}, "funding_rounds": [{{"round": "type", "amount": "$XM", "date": "YYYY-MM", "investors": ["name"], "post_money_valuation": "$XM", "revenue_at_deal": "$XM", "source_id": "id", "source_url": "url"}}], "aov_estimate": {{"value": "$X", "source_id": "id", "source_url": "url"}}, "repeat_purchase_rate": {{"value": "X%", "source_id": "id", "source_url": "url"}}, "revenue_channels": {{"dtc_pct": "X%", "wholesale_pct": "X%", "source_id": "id", "source_url": "url"}}, "geographic_revenue": [{{"region": "name", "pct": "X%", "source_id": "id", "source_url": "url"}}]}},
  "competitors": {{"direct": [{{"name": "Real Company Name", "revenue_est": "amount", "price_range": "$X-$Y", "differentiator": "description", "market_position": "leader/challenger/niche", "source_id": "id", "source_url": "url"}}], "market_size": {{"tam": "$XB", "sam": "$XB", "som": "$XM", "growth_rate": "X% CAGR", "source_id": "id", "source_url": "url"}}, "industry_trends": ["trend1"], "ma_comparables": [{{"target": "company", "acquirer": "buyer", "year": 2024, "value": "$XM", "ev_revenue": "X.Xx", "source_id": "id", "source_url": "url"}}]}},
  ...
  "customer_sentiment": {{"trustpilot": {{"rating": 0.0, "reviews": 0, "source_id": "id", "source_url": "url"}}, "praise_themes": [{{"theme": "description", "quote": "actual quote"}}], "complaint_themes": [{{"theme": "description", "quote": "actual quote"}}]}},
  "operations": {{"manufacturing": "own factory / contract / mixed", "manufacturing_locations": ["country1"], "logistics": "3PL / in-house", "supply_chain_risks": ["risk1"]}}
}}

CRITICAL: Use PitchBook as PRIMARY for revenue/employees/funding. Use Statista for market size. Use Perplexity for digital/social/reviews. Every numeric value MUST have source_id and source_url."""
    ...
    if gap_queries:
        ...
        if gap_findings:
            ...
            merge_system = """You are a data extraction specialist. Extract specific data from these follow-up research findings. Return ONLY valid JSON. For source_url: use the URLs listed below. Never fabricate URLs."""
            merge_prompt = f"""Fill data gaps for {brand_name}:\n\n{gap_text}\n\nAVAILABLE SOURCE URLs:\n{gap_cite_urls}\n\nReturn JSON with ONLY the fields where you found data:\n{{\n  "latest_revenue": {{"year": 2025, "amount": "£XXM", "source_url": "url_or_null"}},\n  "trustpilot": {{"rating": 0.0, "reviews": 0, "source_url": "url_or_null"}},\n  "instagram": {{"followers": "X.XM", "source_url": "url_or_null"}}\n}}"""
...
def _fetch_trustpilot_data(brand_name, domain, pplx_system):
    ...
    Dedicated Trustpilot lookup. Always runs; returns a dict with keys:
      rating (float), reviews (int), praise_themes (list), complaint_themes (list), source_url (str)
    Returns None if nothing useful found.
    """
    log(f"  [Trustpilot] Dedicated lookup for {brand_name} ({domain})...")

    tp_query_1 = f'site:trustpilot.com/review/{domain} rating reviews "{brand_name}"'
    tp_query_2 = f'Trustpilot {brand_name} star rating total reviews customer feedback 2025'
    tp_query_3 = f'"{brand_name}" trustpilot.com review score out of 5'
...
{{
  "rating": 4.2,
  "reviews": 12500,
  "praise_themes": [{{"theme": "Fast delivery", "quote": "example quote or null"}}, {{"theme": "Great quality", "quote": "example quote or null"}}, {{"theme": "Easy returns", "quote": "example quote or null"}}],
  "complaint_themes": [{{"theme": "Sizing issues", "quote": "example quote or null"}}, {{"theme": "Slow customer service", "quote": "example quote or null"}}, {{"theme": "Delivery delays", "quote": "example quote or null"}}],
...
- If Trustpilot page not found, set rating and reviews to null"""
    ...
    log(f"  [Trustpilot] Found: {result.get('rating')} stars, {result.get('reviews')} reviews")
    return result
...
def _check_revenue_staleness(research, brand_name, domain, pplx_system):
    ...
    if age_years <= 1:
        log(f"  [Staleness] Revenue year {rev_year_int} is fresh enough (age: {age_years}y) — no override needed.")
        return research

    log(f"  [Staleness] Revenue year {rev_year_int} is stale (age: {age_years}y). Searching for FY{current_year} / FY{current_year - 1} data...")
    ...
    for i, url in enumerate(cites):
        ...
        if best_url is None and any(domain_hint in url for domain_hint in ["fashionunited", "theindustry", "businessoffashion", "bloomberg", "ft.com", "reuters", "companieshouse", "fashionnetwork"]):
            best_url = url
...
- amount must include the currency symbol and unit (M, B, K)
...
- If you cannot find clear recent revenue data, return {{"year": null, "amount": null}}"""
    ...
    if new_year and new_year > rev_year_int:
        log(f"  [Staleness] Overriding revenue: {latest_rev.get('amount')} ({rev_year_int}) → {new_amount} ({new_year})")
        research["financials"]["latest_revenue"] = {
            ...
            "source_note": f"Updated from web research — PitchBook data was stale (was FY{rev_year_int}: {latest_rev.get('amount')})",
        }
        ...
        if new_year not in existing_years:
            rev_history.append({"year": new_year, "revenue": new_amount, "growth_yoy": None, "source_id": "PPLX-staleness", "source_url": new_url, "source_note": "Updated from web research"})
            research["financials"]["revenue_history"] = sorted(rev_history, key=lambda r: r.get("year") or 0)
    else:
        log(f"  [Staleness] New data (year={new_year}) is not newer than existing (year={rev_year_int}) — keeping original.")
...
def _validate_and_clean_sources(research, brand_name, domain, pplx_system):
    ...
    Steps: 1. Flag low-quality source URLs. 2. Revenue staleness check. 3. Dedicated Trustpilot lookup.
    """
    BAD_SOURCE_PATTERNS = ["1stformations.co.uk", "companieshouse.gov.uk/search", "linkedin.com/company", "crunchbase.com", "growjo.com", "zoominfo.com", "dnb.com", "owler.com", "craft.co", "macrotrends.net", "wisesheets.io", "simplywall.st"]
    ...
    if tp_data:
        ...
        if existing_tp and existing_tp.get("rating") and existing_tp.get("rating") != tp_data.get("rating"):
            log(f"  [Trustpilot] Overrode existing data (was {existing_tp.get('rating')} → now {tp_data.get('rating')})")
        if existing_tp.get("praise_themes") and not tp_data.get("praise_themes"):
            research["customer_sentiment"]["trustpilot"]["praise_themes"] = existing_tp["praise_themes"]
        if existing_tp.get("complaint_themes") and not tp_data.get("complaint_themes"):
            research["customer_sentiment"]["trustpilot"]["complaint_themes"] = existing_tp["complaint_themes"]
    else:
...
- Ownership structure in a stat-row table
...
- A Chart.js radar chart: competitive positioning (5 dimensions: Price, Quality, Brand, Digital, Growth)""",
        "s06": "Write AI & Innovation section with current AI/tech investments, innovation pipeline, AI readiness score with metric bars, comparison vs competitors on AI adoption, and a Chart.js bar chart: AI adoption maturity vs top 5 competitors.",
        "s07": "Write Risk Assessment section with risk matrix table (category, severity, likelihood, mitigation), top 3 risks in callout.danger boxes, key watch items in callout.warn boxes, and a Chart.js bubble chart: risk severity vs likelihood.",
        "s08": "Write Channel Economics section with revenue by channel (DTC, wholesale, marketplace) with margins, stat-rows for gross margin per channel, channel mix trend, and a Chart.js grouped bar chart: revenue and margin by channel.",
        "s09": "Write Cohort Analysis section with customer cohort retention table (12, 24, 36 month rates), payback period analysis, LTV cohort curves, repeat purchase rate stat-row, and a Chart.js line chart: cohort retention curves (month 1-24).",
        "s10": "Write TAM/SAM/SOM section with TAM/SAM/SOM sources and growth rates, market sizing methodology, 3-5 key market trends, geographic opportunity map table, a Chart.js doughnut: TAM/SAM/SOM, and a Chart.js line chart: market size projection (5 years).",
        "s11": "Write Customer Sentiment section with Trustpilot score prominently (large KPI card), top 3 praise themes with customer quotes in blockquotes, top 3 complaint themes with quotes, NPS estimate, sentiment vs competitors comparison, a Chart.js bar chart: star rating distribution, and a Chart.js doughnut: sentiment split.",
        "s12": "Write Content Strategy Gap section with current content assessment, SEO content gap analysis, competitor content benchmarking, recommendations in callout.success boxes, and a Chart.js radar chart: content maturity across 6 dimensions.",
        "s13": "Write Value Creation section with 100-day quick wins table, 12-month priorities with revenue impact, 3-year transformation roadmap, EBITDA bridge, and a Chart.js waterfall bar chart: value creation (base EBITDA to exit EBITDA).",
        "s14": "Write Pricing Strategy section with current price architecture table, price vs competitor comparison, price elasticity discussion, pricing power indicators, recommendations in callout.success, and a Chart.js scatter chart: price vs quality positioning vs competitors.",
        "s15": "Write Revenue Quality section with revenue concentration analysis, recurring vs one-time split, revenue predictability score, seasonality patterns, and a Chart.js line chart: monthly revenue seasonality index.",
        "s16": "Write Management & Org section with leadership team table (name, role, tenure, background, equity), org structure, key person risk, management depth, hiring gaps, and a Chart.js doughnut: org split by department headcount.",
        "s17": "Write Technology Stack section with current tech stack by category, technical debt assessment, scalability rating per system, integration complexity, build vs buy recommendations, and a Chart.js radar chart: tech stack maturity (6 dimensions).",
        "s18": "Write Brand Equity section with brand awareness metrics, brand search volume trend, brand sentiment score, share of voice, brand vs private label premium, and a Chart.js line chart: brand search volume trend (24 months, indexed to 100).",
        "s19": "Write Supply Chain section with manufacturing model, locations table (country, % of production, lead time, risk level), supplier concentration risk, logistics/3PL model, supply chain resilience score, and a Chart.js doughnut: manufacturing geography split.",
        "s20": "Write Regulatory section with applicable frameworks (GDPR, CCPA, product safety, sustainability), compliance status per framework, upcoming regulatory risks (2025-2027), ESG commitments, regulatory tail risks in callout.warn, and a Chart.js bar chart: compliance maturity by regulatory area.",
        "s21": "Write Working Capital section with DSO, DIO, DPO, CCC metrics, working capital as % of revenue, inventory analysis, cash conversion cycle, improvement opportunities in callout.success, and a Chart.js bar chart: working capital components and CCC trend.",
        "s22": "Write Exit Analysis section with exit route options (strategic trade sale, sponsor-to-sponsor, IPO), comparable exit valuations, buyer universe, timeline to exit, valuation upside scenarios, and a Chart.js bar chart: exit valuation bridge by scenario.",
        "s23": "Write Geo Expansion section with current geographic revenue split, expansion priority ranking, US market deep-dive, EU/APAC opportunities, recommended expansion sequence, and a Chart.js bar chart: revenue by geography with expansion potential overlay.",
        "s24": "Write LTV Model section with LTV calculation methodology, AOV/purchase frequency/gross margin/churn, LTV by customer segment, LTV trend over time, LTV benchmarking vs competitors, and a Chart.js line chart: cumulative LTV curve over 36 months.",
        "s25": "Write CAC Payback section with blended CAC by channel (table: channel, CAC, LTV, LTV/CAC ratio, payback months), CAC trend, efficiency improvements, payback period by channel, and a Chart.js grouped bar chart: CAC vs 12-month revenue per customer by channel.",
        "s26": "Write Contribution Margin section with contribution margin P&L (revenue, COGS, gross margin, variable marketing, net contribution), CM by channel, CM by product category, threshold analysis, and a Chart.js waterfall bar chart: gross revenue to net contribution.",
        "s27": "Write Marketing P&L section with full marketing P&L (spend by channel, revenue attributed, ROAS, net contribution), marketing efficiency ratio trend, budget allocation vs recommended, media mix notes, and a Chart.js grouped bar chart: marketing spend vs revenue attributed by channel.",
        "s28": "Write RFM Segmentation section with RFM model description, customer segments table (Champions, Loyal, At-Risk, Lost, New with size and revenue %), segment-specific action recommendations, revenue concentration by RFM tier, and a Chart.js doughnut: customer revenue by RFM segment.",
        "s29": "Write Retention Curves section with 30/60/90/180/365 day retention rates, cohort retention heatmap description, churn analysis by channel/product/geography, win-back campaign effectiveness, retention benchmark vs industry, and a Chart.js line chart: retention rate at 1,3,6,12,18,24 months.",
        "s30": "Write AOV Dynamics section with AOV trend (last 8 quarters), AOV by channel, AOV by product category, cross-sell and upsell attachment rates, price increase vs unit volume analysis, and a Chart.js line chart: AOV trend over last 8 quarters.",
        "s31": "Write NPS & VoC section with NPS score with benchmarks, Voice of Customer themes from reviews, CSAT score, customer feedback loop description, improvement recommendations, and a Chart.js bar chart: top customer feedback themes.",
        "s32": "Write Journey Mapping section with customer journey stages (Awareness to Advocacy), key touchpoints per stage, friction points in callout.warn, optimization opportunities, and a Chart.js line chart: conversion funnel (awareness to purchase, drop-off %).",
        "s33": "Write SEO Authority section with domain authority/rating, organic traffic estimate and trend, top organic keyword categories, backlink profile, SEO vs top 3 competitors comparison, keyword ranking distribution, and a Chart.js bar chart: organic vs paid traffic split comparison vs competitors.",
        "s34": "Write Paid Media section with estimated paid media spend by channel, ROAS by channel, paid vs organic ratio, ad creative strategy assessment, paid media efficiency benchmarks, and a Chart.js doughnut: paid media budget allocation by channel.",
        "s35": "Write Email & CRM section with email list size estimate, key email metrics (open rate, click rate, unsubscribe rate vs benchmarks), CRM platform and capabilities, automation and segmentation maturity, email revenue contribution, and a Chart.js bar chart: email performance vs industry benchmarks.",
        "s36": "Write CRO Analysis section with site conversion rate estimate vs benchmarks, checkout funnel drop-off analysis, mobile vs desktop conversion, A/B testing cadence, top 5 CRO opportunities ranked by impact, and a Chart.js bar chart: conversion funnel steps with drop-off rates.",
        "s37": "Write Social Commerce section with Instagram Shop/TikTok Shop presence, social commerce revenue estimate, influencer marketing program assessment, UGC volume and quality, social commerce growth opportunity, and a Chart.js doughnut: social media revenue split by platform.",
        "s38": "Write Share of Voice section with share of voice in organic search vs top 5 competitors, social media share of voice, branded vs non-branded search share, PR/earned media volume, and a Chart.js bar chart: share of voice comparison vs top 4 competitors.",
        "s39": "Write Price Elasticity section with price elasticity estimate, historical price change impacts, elasticity by product category, pricing power indicators (brand loyalty, switching costs), recommended pricing strategy, and a Chart.js line chart: price index vs volume index (last 8 quarters).",
        "s40": "Write Disruption Threats section with top 3 disruption scenarios (probability and impact), new entrants and challenger brands, technology disruption risks, business model disruption risks, mitigation strategies in callout.success, and a Chart.js bubble chart: disruption scenarios (probability vs impact).",
        "s41": "Write Cross-Border section with current international revenue % and countries, cross-border ecommerce opportunities (US, EU, APAC), localization requirements, regulatory barriers per market, cross-border expansion investment estimate, and a Chart.js bar chart: cross-border market opportunity by region.",
        "s42": "Write IP & Trademark section with trademark registrations (jurisdictions covered, key marks), patent portfolio (if applicable), brand protection risks, IP valuation estimate, recommended IP protections, and a Chart.js doughnut: trademark coverage by key market.",
        "s43": "Write Data Assets section with customer data asset description (size, depth, consent status), first-party data strategy, data monetization potential, data infrastructure and quality, GDPR/CCPA compliance status, and a Chart.js bar chart: data asset maturity scores by dimension.",
        "s44": "Write Content Library section with content asset inventory (photos, videos, blog posts, lookbooks), content production cadence and cost, content performance metrics, content gaps vs competitors, content repurposing opportunities, and a Chart.js doughnut: content mix by type.",
        "s45": "Write MarTech Stack section with full MarTech stack inventory (email, analytics, CRM, CDP, attribution), stack cost estimate, integration quality assessment, stack gaps vs best-in-class, consolidation/migration opportunities, and a Chart.js radar chart: MarTech capability maturity (6 dimensions).",
        "s46": "Write 100-Day Plan section with Days 1-30 quick wins table (action, owner, KPI, cost), Days 31-60 foundation builds table, Days 61-100 growth initiatives table, KPI dashboard targets at Day 100, and a Chart.js gantt-style bar chart: 100-day initiative timeline.",
        "s47": "Write EBITDA Bridge section with entry EBITDA (current), revenue growth contribution, gross margin expansion, OpEx efficiency, new channel contributions, exit EBITDA, detailed waterfall table, and a Chart.js waterfall bar chart: EBITDA bridge from entry to exit.",
        "s48": "Write Scenario Analysis section with bear/base/bull case assumptions table, revenue projections for each scenario (3-5 years), EBITDA margin progression, exit valuation range, key scenario drivers and sensitivities, three .scenario-card.bear/.base/.bull with MOIC and IRR, and a Chart.js line chart: revenue projection by scenario (5 years).",
        "s49": "Write IC Summary section with investment thesis in 3 bullet points, deal structure recommendation (equity, debt, governance), key conditions precedent, recommended advisors for next-phase DD, Go/No-Go recommendation with rationale, key open questions for management, and a final KPI grid with 8 key metrics.",
        "s50": "Write AI Readiness section with AI readiness score (overall and by dimension), current AI/ML capabilities, AI opportunity map (personalization, forecasting, content, customer service), build vs buy vs partner recommendation per use case, investment required to reach AI-ready state, and a Chart.js radar chart: AI readiness across 8 dimensions.",
        "s51": "Write Appendix section with data sources summary table (source name, type, date accessed, URL), methodology notes, glossary of key terms, key assumptions, disclaimer text, and a sources table with all referenced URLs.",
    }
...
def run_report_generation(research, brand_name, domain, report_id):
    ...
    for src in source_registry:
        source_map_lines.append(f"  [{src['id']}] {src['name']} — {src.get('url', 'no url')}")
    ...
    research_context = json.dumps({"company": company, "financials": financials, "competitors": competitors, "digital_marketing": digital, "customer_sentiment": sentiment, "operations": operations}, indent=2)
...
- Output ONLY the HTML — no preamble, no explanation, no markdown."""
  ...
  <div class="callout danger">
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        futures = {executor.submit(generate_batch, i + 1, BATCHES[i]): i + 1 for i in range(5)}
        for future in as_completed(futures):
    ...
    wave2_results = {}
    with ThreadPoolExecutor(max_workers=5) as executor:
        futures = {executor.submit(generate_batch, i + 6, BATCHES[i + 5]): i + 6 for i in range(5)}
        for future in as_completed(futures):
            batch_num, html = future.result()
    ...
    log(f"Phase 2 complete: {sum(len(b) for b in ordered_batches):,} total chars across {len(ordered_batches)} batches")
...
_NAV_SECTIONS = [
    ("s01", "01", "Executive Summary"), ("s02", "02", "Company Profile"), ("s03", "03", "PE Economics"),
    ("s04", "04", "Digital Marketing"), ("s05", "05", "Competitive Intelligence"), ("s06", "06", "AI & Innovation"),
    ("s07", "07", "Risk Assessment"), ("s08", "08", "Channel Economics"), ("s09", "09", "Cohort Analysis"),
    ("s10", "10", "TAM/SAM/SOM"), ("s11", "11", "Customer Sentiment"), ("s12", "12", "Content Strategy Gap"),
    ("s13", "13", "Value Creation"), ("s14", "14", "Pricing Strategy"), ("s15", "15", "Revenue Quality"),
    ("s16", "16", "Management & Org"), ("s17", "17", "Technology Stack"), ("s18", "18", "Brand Equity"),
    ("s19", "19", "Supply Chain"), ("s20", "20", "Regulatory"), ("s21", "21", "Working Capital"),
    ("s22", "22", "Exit Analysis"), ("s23", "23", "Geo Expansion"), ("s24", "24", "LTV Model"),
    ("s25", "25", "CAC Payback"), ("s26", "26", "Contribution Margin"), ("s27", "27", "Marketing P&L"),
    ("s28", "28", "RFM Segmentation"), ("s29", "29", "Retention Curves"), ("s30", "30", "AOV Dynamics"),
    ("s31", "31", "NPS & VoC"), ("s32", "32", "Journey Mapping"), ("s33", "33", "SEO Authority"),
    ("s34", "34", "Paid Media"), ("s35", "35", "Email & CRM"), ("s36", "36", "CRO Analysis"),
    ("s37", "37", "Social Commerce"), ("s38", "38", "Share of Voice"), ("s39", "39", "Price Elasticity"),
    ("s40", "40", "Disruption Threats"), ("s41", "41", "Cross-Border"), ("s42", "42", "IP & Trademark"),
    ("s43", "43", "Data Assets"), ("s44", "44", "Content Library"), ("s45", "45", "MarTech Stack"),
    ("s46", "46", "100-Day Plan"), ("s47", "47", "EBITDA Bridge"), ("s48", "48", "Scenario Analysis"),
    ("s49", "49", "IC Summary"), ("s50", "50", "AI Readiness"), ("s51", "51", "Appendix"),
]
...
def _build_sidebar_nav():
    ...
    for sid, num, title in _NAV_SECTIONS:
        links.append(f'<a href="#{sid}"><span class="nav-num">{num}</span>{title}</a>')
    return "\n        ".join(links)
...
def _linkify_sources(html_text, registry):
    ...
    def replace_tag(match):
        ...
        if src_id in source_map:
            ...
            if url:
                return f'<a href="{url}" target="_blank" class="cite" title="{name}">[{src_id}]</a>'
            else:
    ...
    html_text = _re.sub(r'<span[^>]*class="source-tag"[^>]*>\[Source:\s*([^\]]+)\]</span>', replace_tag, html_text)
...
def assemble_html(brand_name, domain, batches, research, report_id):
    ...
    if source_registry:
        ...
        for src in source_registry:
            ...
            if url:
                rows.append(f'<tr><td><code>{src["id"]}</code></td><td>{name}</td><td>{publisher}</td><td><span class="tag tag-{"opp" if src_type == "premium" else "watch"}">{src_type}</span></td><td><a href="{url}" target="_blank" class="cite">{url[:70]}{"\u2026" if len(url) > 70 else ""}</a></td></tr>')
            else:
                rows.append(f'<tr><td><code>{src["id"]}</code></td><td>{name}</td><td>{publisher}</td><td><span class="tag tag-watch">{src_type}</span></td><td><em>No URL</em></td></tr>')
        source_refs_html = f"""<section class="section" id="s51-sources">
  <div class="section-label">Appendix</div>
  <h2>Source Registry</h2>
  <div class="table-wrap">
    <table>
      <thead><tr><th>ID</th><th>Name</th><th>Publisher</th><th>Type</th><th>URL</th></tr></thead>
    ...
    if premium_data.get("pitchbook", 0) > 0:
        badge_parts.append(f'<span class="premium-badge pb-badge">PitchBook <strong>{premium_data["pitchbook"]}</strong></span>')
    if premium_data.get("statista", 0) > 0:
        badge_parts.append(f'<span class="premium-badge st-badge">Statista <strong>{premium_data["statista"]}</strong></span>')
    if premium_data.get("cbinsights", 0) > 0:
        badge_parts.append(f'<span class="premium-badge cb-badge">CB Insights <strong>{premium_data["cbinsights"]}</strong></span>')
...
<head>
  ...
  <style>
:root {{
  --navy: #1a2332; --blue: #2563eb; --green: #16a34a; --amber: #d97706; --red: #dc2626;
  --gray-50: #f8fafc; --gray-100: #f1f5f9; --gray-200: #e2e8f0; --gray-300: #cbd5e1;
  --gray-400: #94a3b8; --gray-500: #64748b; --gray-600: #475569; --gray-700: #334155;
  --gray-800: #1e293b; --sidebar-w: 260px; --header-h: 0px;
}}
*, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
html {{ scroll-behavior: smooth; font-size: 16px; }}
body {{ font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; color: var(--navy); background: white; line-height: 1.7; -webkit-font-smoothing: antialiased; }}
a {{ color: var(--blue); text-decoration: none; }}
a:hover {{ text-decoration: underline; }}
img {{ max-width: 100%; height: auto; display: block; }}
#sidebar {{ position: fixed; top: 0; left: 0; width: var(--sidebar-w); height: 100vh; background: var(--navy); color: #e2e8f0; display: flex; flex-direction: column; z-index: 100; overflow-y: auto; border-right: 1px solid rgba(255,255,255,0.08); }}
.sidebar-header {{ padding: 24px 20px 16px; border-bottom: 1px solid rgba(255,255,255,0.1); }}
.sidebar-logo {{ font-size: 11px; text-transform: uppercase; letter-spacing: 0.15em; color: var(--gray-400); margin-bottom: 6px; }}
.sidebar-title {{ font-size: 15px; font-weight: 700; line-height: 1.3; color: white; }}
.confidential-badge {{ display: inline-block; margin-top: 8px; padding: 2px 8px; font-size: 10px; text-transform: uppercase; letter-spacing: 0.1em; background: rgba(220,38,38,0.2); color: #fca5a5; border-radius: 3px; font-weight: 600; }}
.sidebar-nav {{ flex: 1; padding: 12px 0; overflow-y: auto; }}
.sidebar-nav a {{ display: flex; align-items: center; padding: 7px 20px; font-size: 12.5px; color: var(--gray-400); text-decoration: none; transition: all 0.15s; border-left: 3px solid transparent; line-height: 1.3; }}
...
.nav-num {{ display: inline-block; width: 26px; font-size: 10px; font-weight: 700; color: var(--gray-500); flex-shrink: 0; }}
...
#hamburger {{ display: none; position: fixed; top: 12px; left: 12px; z-index: 200; background: var(--navy); color: white; border: none; padding: 8px 12px; font-size: 20px; border-radius: 6px; cursor: pointer; }}
...
.firm-label {{ font-size: 11px; text-transform: uppercase; letter-spacing: 0.15em; color: var(--gray-500); margin-bottom: 8px; }}
.report-header h1 {{ font-size: 28px; font-weight: 800; line-height: 1.2; color: var(--navy); margin-bottom: 8px; }}
.subtitle {{ font-size: 15px; color: var(--gray-600); margin-bottom: 16px; }}
.report-meta {{ display: flex; flex-wrap: wrap; gap: 8px 24px; font-size: 12px; color: var(--gray-500); }}
.report-meta span {{ padding: 4px 0; }}
.premium-badges {{ display: flex; flex-wrap: wrap; gap: 8px; margin: 16px 0 0; }}
.premium-badge {{ display: inline-flex; align-items: center; padding: 4px 10px; border-radius: 4px; font-size: 11px; font-weight: 500; letter-spacing: 0.02em; gap: 4px; }}
...
.kpi-strip {{ display: flex; flex-wrap: wrap; gap: 0; margin: 24px 0 40px; border: 1px solid var(--gray-200); border-radius: 8px; overflow: hidden; }}
.kpi-strip-item {{ flex: 1 1 160px; padding: 16px 20px; border-right: 1px solid var(--gray-200); background: var(--gray-50); }}
.kpi-strip-item:last-child {{ border-right: none; }}
.kpi-strip-label {{ font-size: 11px; text-transform: uppercase; letter-spacing: 0.08em; color: var(--gray-500); margin-bottom: 4px; }}
.kpi-strip-value {{ font-size: 22px; font-weight: 800; color: var(--navy); line-height: 1.1; margin-bottom: 2px; }}
.kpi-strip-sub {{ font-size: 11px; color: var(--gray-400); }}
.section {{ margin-bottom: 56px; padding-top: 24px; }}
.section-label {{ font-size: 11px; text-transform: uppercase; letter-spacing: 0.15em; color: var(--blue); font-weight: 700; margin-bottom: 4px; }}
.section h2 {{ font-size: 22px; font-weight: 800; color: var(--navy); margin-bottom: 16px; padding-bottom: 12px; border-bottom: 2px solid var(--gray-200); }}
...
.kpi-label {{ font-size: 11px; text-transform: uppercase; letter-spacing: 0.08em; color: var(--gray-500); margin-bottom: 4px; }}
.kpi-value {{ font-size: 28px; font-weight: 800; color: var(--navy); line-height: 1.1; margin-bottom: 4px; }}
.kpi-sub {{ font-size: 12px; color: var(--gray-600); line-height: 1.4; }}
...
.stat-label {{ font-size: 13px; color: var(--gray-500); min-width: 140px; flex-shrink: 0; }}
.stat-value {{ font-size: 14px; font-weight: 700; color: var(--navy); }}
.stat-note {{ font-size: 12px; color: var(--gray-400); margin-left: auto; }}
.metric-bar {{ margin-bottom: 10px; }}
.mb-label {{ display: flex; justify-content: space-between; font-size: 13px; margin-bottom: 4px; }}
.mb-track {{ height: 8px; background: var(--gray-100); border-radius: 4px; overflow: hidden; }}
.mb-fill {{ height: 100%; background: var(--blue); border-radius: 4px; transition: width 0.5s; }}
.mb-fill.green {{ background: var(--green); }} .mb-fill.amber {{ background: var(--amber); }} .mb-fill.red {{ background: var(--red); }}
.table-wrap {{ overflow-x: auto; margin: 16px 0 24px; }}
table {{ width: 100%; border-collapse: collapse; font-size: 13px; }}
thead {{ background: var(--gray-50); }}
th {{ padding: 10px 14px; text-align: left; font-weight: 700; font-size: 11px; text-transform: uppercase; letter-spacing: 0.05em; color: var(--gray-600); border-bottom: 2px solid var(--gray-200); }}
td {{ padding: 10px 14px; border-bottom: 1px solid var(--gray-100); vertical-align: top; line-height: 1.5; }}
tr:hover td {{ background: var(--gray-50); }}
.tag {{ display: inline-block; padding: 2px 8px; border-radius: 3px; font-size: 11px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.04em; white-space: nowrap; }}
.tag-risk {{ background: #fef2f2; color: #b91c1c; }} .tag-opp {{ background: #f0fdf4; color: #15803d; }} .tag-watch {{ background: #fffbeb; color: #b45309; }}
.tag-high {{ background: #fef2f2; color: #b91c1c; }} .tag-med {{ background: #fffbeb; color: #b45309; }} .tag-low {{ background: #f0fdf4; color: #15803d; }}
.callout {{ display: flex; gap: 12px; padding: 16px 20px; border-radius: 8px; margin: 16px 0; font-size: 13px; line-height: 1.6; }}
...
.thesis-box {{ padding: 24px; background: var(--gray-50); border: 1px solid var(--gray-200); border-radius: 8px; font-size: 14px; line-height: 1.8; margin: 16px 0 24px; }}
.key-insight {{ padding: 20px 24px; background: linear-gradient(135deg, #eff6ff 0%, #f0fdf4 100%); border-left: 4px solid var(--blue); border-radius: 0 8px 8px 0; margin: 20px 0; font-size: 14px; line-height: 1.7; }}
.highlight-box {{ padding: 20px 24px; background: var(--gray-50); border: 1px solid var(--gray-200); border-radius: 8px; margin: 16px 0; }}
.scenarios {{ display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px; margin: 16px 0 24px; }}
.scenario-card {{ padding: 24px; border-radius: 8px; text-align: center; border: 1px solid var(--gray-200); }}
...
.s-label {{ font-size: 12px; text-transform: uppercase; letter-spacing: 0.08em; font-weight: 700; margin-bottom: 8px; }}
.bear .s-label {{ color: var(--red); }} .base .s-label {{ color: var(--blue); }} .bull .s-label {{ color: var(--green); }}
.s-moic {{ font-size: 36px; font-weight: 800; line-height: 1; margin-bottom: 8px; }}
.bear .s-moic {{ color: var(--red); }} .base .s-moic {{ color: var(--blue); }} .bull .s-moic {{ color: var(--green); }}
.s-sub {{ font-size: 12px; color: var(--gray-600); line-height: 1.5; }}
.chart-container {{ position: relative; margin: 24px 0; background: var(--gray-50); border: 1px solid var(--gray-200); border-radius: 8px; padding: 20px 20px 12px; overflow: hidden; }}
.chart-container canvas {{ max-height: 320px; width: 100% !important; }}
.chart-container + figcaption {{ font-size: 12px; color: var(--gray-500); line-height: 1.5; padding: 8px 4px 16px; }}
.chart-container + figcaption a {{ color: var(--blue); }}
figcaption {{ font-size: 12px; color: var(--gray-500); line-height: 1.5; padding: 8px 4px 16px; }}
.exhibit {{ margin: 24px 0; background: var(--gray-50); border: 1px solid var(--gray-200); border-radius: 8px; overflow: hidden; }}
.exhibit figcaption {{ padding: 12px 16px; font-size: 12px; color: var(--gray-500); line-height: 1.5; border-top: 1px solid var(--gray-200); }}
.report-list {{ list-style: none; padding: 0; margin: 12px 0; }}
.report-list li {{ padding: 8px 0 8px 20px; position: relative; font-size: 14px; line-height: 1.6; border-bottom: 1px solid var(--gray-100); }}
.report-list li::before {{ content: '\u25b8'; position: absolute; left: 0; color: var(--blue); font-weight: bold; }}
.risk-high {{ color: var(--red); font-weight: 700; }} .risk-medium {{ color: var(--amber); font-weight: 700; }} .risk-low {{ color: var(--green); font-weight: 700; }}
.text-muted {{ color: var(--gray-500); }} .text-blue {{ color: var(--blue); }} .text-green {{ color: var(--green); }} .text-amber {{ color: var(--amber); }} .text-red {{ color: var(--red); }}
.tiny {{ font-size: 12px; }} .small {{ font-size: 13px; }}
a.cite {{ font-size: 10px; color: var(--blue); background: #eff6ff; padding: 1px 5px; border-radius: 3px; text-decoration: none; white-space: nowrap; border: 1px solid #bfdbfe; vertical-align: middle; }}
a.cite:hover {{ background: #dbeafe; text-decoration: none; }}
.no-source {{ font-size: 10px; color: var(--gray-400); background: var(--gray-100); padding: 1px 5px; border-radius: 3px; white-space: nowrap; vertical-align: middle; }}
.report-gate-overlay {{ position: fixed; inset: 0; background: var(--navy); z-index: 500; display: flex; align-items: center; justify-content: center; }}
.report-gate-card {{ background: white; border-radius: 12px; padding: 48px; max-width: 440px; width: 90%; text-align: center; box-shadow: 0 24px 48px rgba(0,0,0,0.3); }}
.report-gate-card .gate-brand {{ font-size: 11px; text-transform: uppercase; letter-spacing: 0.15em; color: var(--gray-400); margin-bottom: 16px; font-weight: 700; }}
.report-gate-card h2 {{ font-size: 24px; font-weight: 800; color: var(--navy); margin-bottom: 8px; border: none; padding: 0; }}
.report-gate-card p {{ font-size: 14px; color: var(--gray-500); margin-bottom: 24px; }}
.report-gate-form {{ display: flex; flex-direction: column; gap: 12px; }}
.report-gate-form input {{ padding: 14px 16px; border: 2px solid var(--gray-200); border-radius: 8px; font-size: 16px; text-align: center; letter-spacing: 0.05em; outline: none; transition: border-color 0.2s; }}
.report-gate-form input:focus {{ border-color: var(--blue); }}
.report-gate-form button {{ padding: 14px; background: var(--navy); color: white; border: none; border-radius: 8px; font-size: 15px; font-weight: 700; cursor: pointer; transition: background 0.2s; }}
.report-gate-form button:hover {{ background: #243347; }}
.report-gate-status {{ font-size: 13px; margin-top: 8px; min-height: 20px; }}
.report-gate-status.error {{ color: var(--red); }}
.gate-hint {{ margin-top: 20px; padding-top: 16px; border-top: 1px solid var(--gray-200); font-size: 12px; color: var(--gray-400); }}
.gate-hint strong {{ color: var(--navy); font-size: 14px; letter-spacing: 0.05em; }}
.gate-back {{ display: inline-block; margin-top: 12px; font-size: 13px; color: var(--gray-400); }}
#lightbox-overlay {{ display: none; position: fixed; inset: 0; background: rgba(0,0,0,0.85); z-index: 1000; justify-content: center; align-items: center; cursor: zoom-out; }}
#lightbox-overlay.active {{ display: flex; flex-direction: column; }}
#lightbox-close {{ position: fixed; top: 16px; right: 16px; background: none; border: none; color: white; font-size: 36px; cursor: pointer; z-index: 1001; line-height: 1; }}
#lightbox-img {{ max-width: 90vw; max-height: 85vh; object-fit: contain; border-radius: 4px; }}
#lightbox-caption {{ color: #cbd5e1; font-size: 13px; text-align: center; max-width: 80vw; margin-top: 12px; line-height: 1.5; }}
.report-footer {{ margin-left: var(--sidebar-w); padding: 24px 56px; border-top: 1px solid var(--gray-200); font-size: 12px; color: var(--gray-400); display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 8px; }}
...
<div class="report-gate-overlay" id="reportGate">
  <div class="report-gate-card">
    ...
    <div class="gate-hint">
      ...
      <a href="#" class="gate-back" onclick="document.getElementById('reportGate').style.display='none';return false;">Skip (demo mode)</a>
    </div>
  </div>
</div>
...
<div id="main">
  ...
  <div class="kpi-strip">
    <div class="kpi-strip-item"><div class="kpi-strip-label">Latest Revenue</div><div class="kpi-strip-value">{rev_amount}</div><div class="kpi-strip-sub">FY{rev_year}{rev_source_html}</div></div>
    <div class="kpi-strip-item"><div class="kpi-strip-label">Gross Margin</div><div class="kpi-strip-value">{gross_margin}</div><div class="kpi-strip-sub">reported</div></div>
    <div class="kpi-strip-item"><div class="kpi-strip-label">EBITDA</div><div class="kpi-strip-value">{ebitda_amount}</div><div class="kpi-strip-sub">{ebitda_margin} margin</div></div>
    <div class="kpi-strip-item"><div class="kpi-strip-label">Employees</div><div class="kpi-strip-value">{emp_val}</div><div class="kpi-strip-sub">est.</div></div>
    <div class="kpi-strip-item"><div class="kpi-strip-label">Founded</div><div class="kpi-strip-value">{founded_year}</div><div class="kpi-strip-sub">{hq}</div></div>
    <div class="kpi-strip-item"><div class="kpi-strip-label">Trustpilot</div><div class="kpi-strip-value">{tp_rating}<span style="font-size:14px;font-weight:400;color:var(--gray-400)">/5</span></div><div class="kpi-strip-sub">{tp_reviews_fmt} reviews</div></div>
  </div>
  {sections_html}
  {source_refs_html}
</div>
...
(function() {{
  ...
  chartContainers.forEach(function(container) {{
    ...
    try {{ config = JSON.parse(rawConfig); }} catch(e) {{
      ...
      container.innerHTML = '<div style="padding:16px;color:var(--gray-400);font-size:13px;">Chart data unavailable (JSON parse error)</div>';
      return;
    }}
    ...
    if (!config.options.plugins.legend) {{ config.options.plugins.legend = {{ position: 'bottom', labels: {{ font: {{ size: 11 }} }} }}; }}
    ...
    if (config.data && config.data.datasets) {{
      config.data.datasets.forEach(function(ds, idx) {{
        ...
        if (!ds.backgroundColor) {{
          if (chartType === 'line') {{ ds.backgroundColor = ca; }}
          else if (chartType === 'doughnut' || chartType === 'pie' || chartType === 'polarArea') {{ ds.backgroundColor = palette.slice(0, (ds.data || []).length); }}
          else {{ ds.backgroundColor = c; }}
        }}
        if (!ds.borderColor && chartType === 'line') {{
          ds.borderColor = c; ds.borderWidth = ds.borderWidth || 2; ds.tension = ds.tension !== undefined ? ds.tension : 0.35;
          ds.fill = ds.fill !== undefined ? ds.fill : false; ds.pointRadius = ds.pointRadius !== undefined ? ds.pointRadius : 3;
        }}
      }});
    }}
    try {{ new Chart(canvas.getContext('2d'), config); }} catch(e) {{
      console.warn('Chart ' + chartIndex + ': render error —', e.message);
      container.innerHTML = '<div style="padding:16px;color:var(--gray-400);font-size:13px;">Chart render error: ' + e.message + '</div>';
    }}
  }});
